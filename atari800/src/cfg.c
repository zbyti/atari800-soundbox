	...
	while (fgets(string, sizeof(string), fp)) {
		...
		if (ptr != NULL) {
			*ptr++ = '\0';
			...
			else if (RESID_ReadConfig(string, ptr)) {
			}
#endif
#ifdef SLIGHTSID
			else if (SLIGHTSID_ReadConfig(string, ptr)) {
			}
#endif
#ifdef EVIE
			else if (EVIE_ReadConfig(string, ptr)) {
			}
#endif
#ifdef SIDARI
			else if (SIDARI_ReadConfig(string, ptr)) {
			}
#endif
#ifdef SONARI
			else if (SONARI_ReadConfig(string, ptr)) {
			}
#endif
#ifdef MELODY_PSG
			else if (MELODY_PSG_ReadConfig(string, ptr)) {
			}
#endif
#ifdef YAMARI
			else if (YAMARI_ReadConfig(string, ptr)) {
			}
#endif
#ifdef XEP80_EMULATION
...
#if defined(SLIGHTSID) || defined(EVIE) || defined(SIDARI)
	RESID_WriteConfig(fp);
#endif
#ifdef SLIGHTSID
	SLIGHTSID_WriteConfig(fp);
#endif
#ifdef EVIE
	EVIE_WriteConfig(fp);
#endif
#ifdef SIDARI
	SIDARI_WriteConfig(fp);
#endif
#ifdef SONARI
	SONARI_WriteConfig(fp);
#endif
#ifdef MELODY_PSG
	MELODY_PSG_WriteConfig(fp);
#endif
#ifdef YAMARI
	YAMARI_WriteConfig(fp);
#endif
#ifdef XEP80_EMULATION
