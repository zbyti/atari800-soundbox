...
/* skip "ROM" on systems that are known to be case-insensitive */
	...
	*argc = j;
	...
	if (!SYSROM_Initialise(argc, argv)
		...
		|| !RESID_Initialise(argc, argv)
#endif
#ifdef SLIGHTSID
		|| !SLIGHTSID_Initialise(argc, argv)
#endif
#ifdef EVIE
		|| !EVIE_Initialise(argc, argv)
#endif
#ifdef SIDARI
		|| !SIDARI_Initialise(argc, argv)
#endif
#ifdef SONARI
		|| !SONARI_Initialise(argc, argv)
#endif
#ifdef MELODY_PSG
		|| !MELODY_PSG_Initialise(argc, argv)
#endif
#ifdef YAMARI
		|| !YAMARI_Initialise(argc, argv)
#endif
#ifndef BASIC
