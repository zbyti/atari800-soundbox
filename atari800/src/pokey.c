...
UBYTE POKEY_SKCTL[POKEY_MAXPOKEYS];
UBYTE POKEY_SKSTAT[POKEY_MAXPOKEYS];
UBYTE POKEY_IRQST[POKEY_MAXPOKEYS];
UBYTE POKEY_IRQEN[POKEY_MAXPOKEYS];
...
UBYTE POKEY_GetByte(UWORD addr, int no_side_effects)
{
	...
	if ((EVIE_version != EVIE_NO) && (addr >= 0xd280)) {
		return EVIE_D2GetByte(addr, no_side_effects);
	}
#endif
	...
	case POKEY_OFFSET_SKSTAT:
		byte = POKEY_SKSTAT[0] + (CASSETTE_IOLineStatus() << 4);
#ifdef VOICEBOX
		if (VOICEBOX_enabled) {
			byte = POKEY_SKSTAT[0] + (VOTRAXSND_busy  << 4);
		}
#endif
		break;
	}
...
static int POKEY_siocheck(void)
{
	return (((POKEY_AUDF[POKEY_CHAN3] == 0x28 || POKEY_AUDF[POKEY_CHAN3] == 0x10
		...
		|| (POKEY_SKCTL[0] & 0x78) == 0x28) /* cassette save mode */
...
#ifdef POKEY_DUMP_LOG
	Log_print("%02x%02x%02x %03x %02x %02x", MEMORY_SafeGetByte((UWORD) (0x12+0)), MEMORY_SafeGetByte((UWORD) (0x12+1)), MEMORY_SafeGetByte((UWORD) (0x12+2)), ANTIC_ypos, addr, byte);
#endif
...
#endif
		POKEY_IRQST[0] |= ~byte & 0xf7;	/* Reset disabled IRQs except XMTDONE */
		if ((~POKEY_IRQST[0] & POKEY_IRQEN[0]) == 0 && PBI_IRQ == 0 && PIA_IRQ == 0)
			CPU_IRQ = 0;
		else
			CPU_GenerateIRQ();
		break;
...
#endif
		if ((POKEY_SKCTL[0] & 0x70) == 0x20 && POKEY_siocheck())
			SIO_PutByte(byte);
		/* check if cassette 2-tone mode has been enabled */
		if ((POKEY_SKCTL[0] & 0x08) == 0x00) {
			/* intelligent device */
...
int POKEY_Initialise(int *argc, char *argv[])
{
	...
	POKEY_clock_freq = Atari800_tv_mode == Atari800_TV_PAL ? 1773447.0 : 1789790.0;
...
void POKEY_Scanline(void)
{
	...
	if (POKEY_DELAYED_SERIN_IRQ > 0) {
		if (--POKEY_DELAYED_SERIN_IRQ == 0) {
			...
			if (POKEY_IRQEN[0] & 0x20) {
				if (POKEY_IRQST[0] & 0x20) {
					POKEY_IRQST[0] &= 0xdf;
#ifdef DEBUG2
...
void POKEY_StateSave(void)
{
	...
	StateSav_SaveUBYTE(&POKEY_IRQST[0], 1);
	StateSav_SaveUBYTE(&POKEY_IRQEN[0], 1);
	StateSav_SaveUBYTE(&POKEY_SKCTL[0], 1);
...
void POKEY_StateRead(void)
{
	...
	StateSav_ReadUBYTE(&POKEY_IRQST[0], 1);
	StateSav_ReadUBYTE(&POKEY_IRQEN[0], 1);
	StateSav_ReadUBYTE(&POKEY_SKCTL[0], 1);
