...
#ifdef SYNCHRONIZED_SOUND
static unsigned int generate_sync(UBYTE *buffer_begin, UBYTE *buffer_end, unsigned int num_ticks);
...
static void Update_consol_sound_mz( int set )
{
	...
	if (set) { /* The set variable is 0 only in VOL_ONLY_SOUND routines */
		...
		if (EVIE_version != EVIE_NO) {
			pokey_states[1].speaker = GTIA_speaker*CONSOLE_VOL;
			pokey_states[1].forcero = 1; /* second chip */
		}
#endif
	}
