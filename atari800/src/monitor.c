...
static void make_flags(char *str, const char *flags, UBYTE reg)
{
	int i;
	int l = strlen(flags);
	for (i = 0; i < l; i++)
		str[i] = reg & (1 << (l-1-i)) ? flags[i] : '_';
}

static const double A4 = 440.0; /* ISO-16 base A-4 frequency */
...
static const char audio_flags[] = "9qQjJfFc";
static const char serialcontrol_flags[] = "S***TAsd";
static const char serialstatus_flags[] = "fksS!Ki-";
static const char irq_flags[] = "BKioe421";
...
static const int distortion_map[] = {
	0, /* 5+17 */
	8, /* level */
	5, /* 5 */
	8, /* level */
...
static void print_POKEY(int no, int off, double q)
{
	double qbase;
	int div;
	double q1;
	...
	div = (POKEY_AUDCTL[no] & POKEY_CLOCK_15) ? POKEY_DIV_15 : POKEY_DIV_64;
	qbase = q / div;

	s12 = (POKEY_SKCTL[no] & 0x88) == 0x88;

	/* TODO: pwm calculations:
	 * 1. ch12/ch34 - pwm calculated for ch1/ch3 based on ch2/ch4 period
	 * 2. s12 - pwm calculated for ch1 based on ch2
	 * 3. s12+flt3 - pwm calculated in ch1 based on ch2 and ch3 period
	 */
	switch (POKEY_AUDCTL[no] & (POKEY_CH1_CH2 | POKEY_CH1_179)) {
	case POKEY_CH1_CH2 | POKEY_CH1_179:
		q2 = q;
		c2 = 7;
		v2 = POKEY_AUDF[POKEY_CHAN1+off] | (POKEY_AUDF[POKEY_CHAN2+off] << 8);
		p2 = 2 * (v2 + c2);
		w2 = 50.0;

		q1 = q;
		c1 = 7;
		v1 = POKEY_AUDF[POKEY_CHAN1+off];
		if (s12) {
			p1 = v2 + c2;
			if ((v1 + c1) >= p1) {
				w1 = 100.0;
	...
	if (POKEY_AUDC[POKEY_CHAN2+off] & POKEY_VOL_ONLY) {
		w2 = 100.0;
	}
	if (POKEY_AUDC[POKEY_CHAN1+off] & POKEY_VOL_ONLY) {
		w1 = 100.0;
	}

	switch (POKEY_AUDCTL[no] & (POKEY_CH3_CH4 | POKEY_CH3_179)) {
	case POKEY_CH3_CH4 | POKEY_CH3_179:
		q4 = q;
		c4 = 7;
		v4 = POKEY_AUDF[POKEY_CHAN3+off] | (POKEY_AUDF[POKEY_CHAN4+off] << 8);
		p4 = 2 * (v4 + c4);
		w4 = 50.0;

		q3 = q;
		c3 = 7;
		v3 = POKEY_AUDF[POKEY_CHAN3+off];
		p3 = 2 * (v3 + c3); /*TODO: calc */
		w3 = 50.0; /*TODO: calc */
		break;
	case POKEY_CH3_CH4:
		q4 = qbase;
	...
	if (POKEY_AUDC[POKEY_CHAN4+off] & POKEY_VOL_ONLY) {
	       w4 = 100.0;
	}
	if (POKEY_AUDC[POKEY_CHAN3+off] & POKEY_VOL_ONLY) {
	       w3 = 100.0;
	}

	f1 = q1 / p1;
	tone1 = 12 * log2(f1 / A4) + 57 + 12 * 1;
	cent1 = (int)(100 * modf(tone1, &tone) + 100 ) % 100;
	note1 = note_name[(int)tone % 12];
	oct1 = ((int)tone / 12) - 1;
	f2 = q2 / p2;
	tone2 = 12 * log2(f2 / A4) + 57 + 12 * 1;
	cent2 = (int)(100 * modf(tone2, &tone) + 100 ) % 100;
	note2 = note_name[(int)tone % 12];
	oct2 = ((int)tone / 12) - 1;
	f3 = q3 / p3;
	tone3 = 12 * log2(f3 / A4) + 57 + 12 * 1;
	cent3 = (int)(100 * modf(tone3, &tone) + 100 ) % 100;
	note3 = note_name[(int)tone % 12];
	oct3 = ((int)tone / 12) - 1;
	f4 = q4 / p4;
	tone4 = 12 * log2(f4 / A4) + 57 + 12 * 1;
	cent4 = (int)(100 * modf(tone4, &tone) + 100 ) % 100;
	note4 = note_name[(int)tone % 12];
	oct4 = ((int)tone / 12) - 1;

	p9 = (POKEY_AUDCTL[no] & 0x80) >> 3;
	d1 = (POKEY_AUDC[POKEY_CHAN1+off] & 0xf0) >> 4;
	d2 = (POKEY_AUDC[POKEY_CHAN2+off] & 0xf0) >> 4;
	d3 = (POKEY_AUDC[POKEY_CHAN3+off] & 0xf0) >> 4;
	d4 = (POKEY_AUDC[POKEY_CHAN4+off] & 0xf0) >> 4;
	n1 = distortion_names[distortion_map[ p9 | d1 ]];
	n2 = distortion_names[distortion_map[ p9 | d2 ]];
	n3 = distortion_names[distortion_map[ p9 | d3 ]];
	n4 = distortion_names[distortion_map[ p9 | d4 ]];

	make_flags(audiocontrol, audio_flags, POKEY_AUDCTL[no]);
	make_flags(serialcontrol, serialcontrol_flags, POKEY_SKCTL[no]);
	make_flags(serialstatus, serialstatus_flags, POKEY_SKSTAT[no] ^ 0xff);
	make_flags(irqcontrol, irq_flags, POKEY_IRQEN[no]);
	make_flags(irqstatus, irq_flags, POKEY_IRQST[no] ^ 0xff);

	printf(" AUDF1= %02X    AUDC1= %02X    dist= %5s  freq= ",
		   POKEY_AUDF[POKEY_CHAN1+off], POKEY_AUDC[POKEY_CHAN1+off], n1);
	printf("%s%+03d.%02d ", note1, oct1, cent1);
	printf("%10.3f (%04X)  ", f1, v1);
	if ((d1 == 0xe) || (d1 == 0xa) || (d1 & 0x01)) {
		printf("pwm= %6.2f\n", w1);
	}
	else {
		printf("pwm= ---\n");
	}
	printf(" AUDF2= %02X    AUDC2= %02X    dist= %5s  freq= ",
		   POKEY_AUDF[POKEY_CHAN2+off], POKEY_AUDC[POKEY_CHAN2+off], n2);
	printf("%s%+03d.%02d ", note2, oct2, cent2);
	printf("%10.3f (%04X)  ", f2, v2);
	if ((d2 == 0xe) || (d2 == 0xa) || (d2 & 0x01)) {
		printf("pwm= %6.2f\n", w2);
	}
	else {
		printf("pwm= ---\n");
	}
	printf(" AUDF3= %02X    AUDC3= %02X    dist= %5s  freq= ",
		   POKEY_AUDF[POKEY_CHAN3+off], POKEY_AUDC[POKEY_CHAN3+off], n3);
	printf("%s%+03d.%02d ", note3, oct3, cent3);
	printf("%10.3f (%04X)  ", f3, v3);
	if ((d3 == 0xe) || (d3 == 0xa) || (d3 & 0x01)) {
		printf("pwm= %6.2f\n", w3);
	}
	else {
		printf("pwm= ---\n");
	}
	printf(" AUDF4= %02X    AUDC4= %02X    dist= %5s  freq= ",
		   POKEY_AUDF[POKEY_CHAN4+off], POKEY_AUDC[POKEY_CHAN4+off], n4);
	printf("%s%+03d.%02d ", note4, oct4, cent4);
	printf("%10.3f (%04X)  ", f4, v4);
	if ((d4 == 0xe) || (d4 == 0xa) || (d4 & 0x01)) {
		printf("pwm= %6.2f\n", w4);
	}
	else {
		printf("pwm= ---\n");
	...
	printf("AUDCTL= %02X                 ctrl= %s\n",
		   POKEY_AUDCTL[no], audiocontrol);
	printf(" SKCTL= %02X   SKSTAT= %02X    ctrl= %s  stat= %s\n", 
		   POKEY_SKCTL[no], POKEY_SKSTAT[no], serialcontrol, serialstatus);
	printf(" IRQEN= %02X    IRQST= %02X    ctrl= %s  stat= %s\n",
		   POKEY_IRQEN[no], POKEY_IRQST[no], irqcontrol, irqstatus);
	if (no == 0) {
		char *control = (POKEY_KBCODE & 0x80) ? "control " : "";
		char *shift = (POKEY_KBCODE & 0x40) ? "shift " : "";
		char *key = key_names[POKEY_KBCODE & 0x3f];
		printf("KBCODE= %02X                  key= %s%s%s\n", 
			   POKEY_KBCODE, control, shift, key);
	}
}
...
{
	print_POKEY(0, POKEY_CHIP1, POKEY_clock_freq);

#ifdef STEREO_SOUND
	if (POKEYSND_stereo_enabled) {
		printf("Second chip:\n");
		print_POKEY(1, POKEY_CHIP2, POKEY_clock_freq);
	}
#endif
}
...
#if defined(SLIGHTSID) || defined(EVIE) || defined(SIDARI)

static const char control_flags[] = "npstTRSG";
static const char filter_flags[] = "X321";
static const char mode_flags[] = "3HBL";
static char *period_attack[] = {
	"2.030",
	"8.120",
	"16.239",
	"24.359",
...
static char *period_decay_release[] = {
	"6.090",
	"24.359",
	"48.718",
	"73.078",
 ...
 * 2^(n/12)=f/440
 * log2(f/440)=n/12
 * n=12*log2(f/440)+9+4*12
 * n=12*log2(f/440)+57
 */
static void print_SID(RESID_State *s, double quartz)
{
	char control1[] = "........";
	char control2[] = "........";
	char control3[] = "........";
	...
	make_flags(control1, control_flags, s->sid_register[0x04]);
	make_flags(control2, control_flags, s->sid_register[0x0b]);
	make_flags(control3, control_flags, s->sid_register[0x12]);

	/* following the "MOS 6581 SID" datasheet */
	f1 = s->sid_register[0x00] | (s->sid_register[0x01] << 8);
	freq1 = f1 * quartz / 0x1000000;
	tone1 = 12 * log2(freq1 / A4) + 57 + 12 * 10;
	cent1 = (int)(100 * modf(tone1, &tone) + 100 ) % 100;
	note1 = note_name[(int)tone % 12];
	oct1 = ((int)tone / 12) - 10;
	f2 = s->sid_register[0x07] | (s->sid_register[0x08] << 8);
	freq2 = f2 * quartz / 0x1000000;
	tone2 = 12 * log2(freq2 / A4) + 57 + 12 * 10;
	cent2 = (int)(100 * modf(tone2, &tone) + 100) % 100;
	note2 = note_name[(int)tone % 12];
	oct2 = ((int)tone / 12) - 10;
	f3 = s->sid_register[0x0e] | (s->sid_register[0x0f] << 8);
	freq3 = f3 * quartz / 0x1000000;
	tone3 = 12 * log2(freq3 / A4) + 57 + 12 * 10;
	cent3 = (int)(100 * modf(tone3, &tone) + 100) % 100;
	note3 = note_name[(int)tone % 12];
	oct3 = ((int)tone / 12) - 10;

	p1 = s->sid_register[0x02] | (s->sid_register[0x03] << 8);
	pwm1 = p1 / 40.95;
	p2 = s->sid_register[0x09] | (s->sid_register[0x0a] << 8);
	pwm2 = p2 / 40.95;
	p3 = s->sid_register[0x10] | (s->sid_register[0x11] << 8);
	pwm3 = p3 / 40.95;

	a1 = period_attack[s->sid_register[0x05] >> 4];
	d1 = period_decay_release[s->sid_register[0x05] & 0x0f];
	s1 = s->sid_register[0x06] >> 4;
	r1 = period_decay_release[s->sid_register[0x06] & 0x0f];
	adsr1 = (s->sid_register[0x05] << 8) | s->sid_register[0x06];
	a2 = period_attack[s->sid_register[0x0c] >> 4];
	d2 = period_decay_release[s->sid_register[0x0c] & 0x0f];
	s2 = s->sid_register[0x0d] >> 4;
	r2 = period_decay_release[s->sid_register[0x0d] & 0x0f];
	adsr2 = (s->sid_register[0x0c] << 8) | s->sid_register[0x0d];
	a3 = period_attack[s->sid_register[0x13] >> 4];
	d3 = period_decay_release[s->sid_register[0x13] & 0x0f];
	s3 = s->sid_register[0x14] >> 4;
	r3 = period_decay_release[s->sid_register[0x14] & 0x0f];
	adsr3 = (s->sid_register[0x13] << 8) | s->sid_register[0x14];

	/* following the "Mapping the 64" book */
	cutoff = (s->sid_register[0x15] & 0x07) | ((SWORD)s->sid_register[0x16] << 3);
	freq = cutoff * 5.8 + 30;
	resonance = s->sid_register[0x17] >> 4;

	make_flags(filter, filter_flags, s->sid_register[0x17]);
	make_flags(mode, mode_flags, (s->sid_register[0x18] ^ 0x80) >> 4);

	printf("FREQLO1= %02X  FREQHI1= %02X  PWLO1= %02X    PWHI1= %02X        freq= ",
		s->sid_register[0x00], s->sid_register[0x01], s->sid_register[0x02], s->sid_register[0x03]);
	if (f1 > 0) {
		printf("%s%+d.%02d ", note1, oct1, cent1);
	}
	else {
		printf("        ");
	}
	printf("%9.3f (%04X)  pwm= %6.2f (%03X)\n", freq1, f1, pwm1, p1);
	printf("    CR1= %02X      AD1= %02X    SR1= %02X    vctrl= %s  adsr= %9s %9s %X %9s (%04X)\n",
		s->sid_register[0x04], s->sid_register[0x05], s->sid_register[0x06], 
		control1, a1, d1, s1, r1, adsr1);
	printf("FREQLO2= %02X  FREQHI2= %02X  PWLO2= %02X    PWHI2= %02X        freq= ",
		s->sid_register[0x07], s->sid_register[0x08], s->sid_register[0x09], s->sid_register[0x0a]);
	if (f2 > 0) {
		printf("%s%+d.%02d ", note2, oct2, cent2);
	}
	else {
		printf("        ");
	}
	printf("%9.3f (%04X)  pwm= %6.2f (%03X)\n", freq2, f2, pwm2, p2);
	printf("    CR2= %02X      AD2= %02X    SR2= %02X    vctrl= %s  adsr= %9s %9s %X %9s (%04X)\n",
		s->sid_register[0x0b], s->sid_register[0x0c], s->sid_register[0x0d], 
		control2, a2, d2, s2, r2, adsr2);
	printf("FREQLO3= %02X  FREQHI3= %02X  PWLO3= %02X    PWHI3= %02X        freq= ",
		s->sid_register[0x0e], s->sid_register[0x0f], s->sid_register[0x10], s->sid_register[0x11]);
	if (f3 > 0) {
		printf("%s%+d.%02d ", note3, oct3, cent3);
	}
	else {
		printf("        ");
	}
	printf("%9.3f (%04X)  pwm= %6.2f (%03X)\n", freq3, f3, pwm3, p3);
	printf("    CR3= %02X      AD3= %02X    SR3= %02X    vctrl= %s  adsr= %9s %9s %X %9s (%04X)\n",
		s->sid_register[0x12], s->sid_register[0x13], s->sid_register[0x14], 
		control3, a3, d3, s3, r3, adsr3);
	printf("   FCLO= %02X     FCHI= %02X     RF= %02X       MV= %02X\n",
		s->sid_register[0x15], s->sid_register[0x16], s->sid_register[0x17], s->sid_register[0x18]);
	printf("           resonance= %X  filter= %s   mode= %s      freq=         %9.3f (%03X)\n", 
		resonance, filter, mode, freq, cutoff);
}
#endif

#ifdef SLIGHTSID
static void show_SlightSID(void)
{
	if (SLIGHTSID_version != SLIGHTSID_NO) {
		RESID_State s;
		double quartz = SLIGHTSID_clock_freq;
		RESID_read_state(RESID_CHIP_SLIGHTSID_LEFT_INDEX, &s);
		print_SID(&s, quartz);
		if (SLIGHTSID_version == SLIGHTSID_STEREO) {
			printf("Second chip:\n");
			RESID_read_state(RESID_CHIP_SLIGHTSID_RIGHT_INDEX, &s);
			print_SID(&s, quartz);
		}
	}
}
...
#if defined(EVIE) || defined(SONARI) || defined(MELODY_PSG)
static const char mixer_flags[] = "CBAcba";
static const char envelope_flags[] = "CAaH";
static const char *envelope_shape[] = {
	"\\___",
	"\\___",
	"\\___",
	"\\___",
...
static void print_PSG(AYEMU_State *s, double quartz)
{
	UWORD pa;
	UWORD pb;
	UWORD pc;
	...
	make_flags(mixer, mixer_flags, ~s->regs[0x07] & 0x3F);
	make_flags(envelope, envelope_flags, s->regs[0x0d] & 0x0F);

	pa = s->regs[0x00] | ((s->regs[0x01] & 0x0f) << 8);
	pb = s->regs[0x02] | ((s->regs[0x03] & 0x0f)<< 8);
	pc = s->regs[0x04] | ((s->regs[0x05] & 0x0f) << 8);
	pn = s->regs[0x06] & 0x1f;
	pe = s->regs[0x0b] | (s->regs[0x0c] << 8);
	fa = quartz / (16 * (pa ? pa : 1));
	tonea = 12 * log2(fa / A4) + 57 + 12 * 8;
	centa = (int)(100 * modf(tonea, &tone) + 100 ) % 100;
	notea = note_name[(int)tone % 12];
	octa = ((int)tone / 12) - 8;
	fb = quartz / (16 * (pb ? pb : 1));
	toneb = 12 * log2(fb / A4) + 57 + 12 * 8;
	centb = (int)(100 * modf(toneb, &tone) + 100 ) % 100;
	noteb = note_name[(((int)tone % 12) + 12) % 12];
	octb = ((int)tone / 12) - 8;
	fc = quartz / (16 * (pc ? pc : 1));
	tonec = 12 * log2(fc / A4) + 57 + 12 * 8;
	centc = (int)(100 * modf(tonec, &tone) + 100 ) % 100;
	notec = note_name[(int)tone % 12];
	octc = ((int)tone / 12) - 8;
	fn = quartz / (16 * (pn ? pn : 1));
	tonen = 12 * log2(fn / A4) + 57 + 12 * 8;
	centn = (int)(100 * modf(tonen, &tone) + 100 ) % 100;
	noten = note_name[(int)tone % 12];
	octn = ((int)tone / 12) - 8;
	fe = quartz / (16*16 * (pe ? pe : 1));
	tonee = 12 * log2(fe / A4) + 57 + 12 * 8;
	cente = (int)(100 * modf(tonee, &tone) + 100 ) % 100;
	notee = note_name[(int)tone % 12];
	octe = ((int)tone / 12) - 8;

	printf("ATONEPLO= %02X  ATONEPHI= %02X  AAMP= %02X     freq= ",
		s->regs[0x00], s->regs[0x01], s->regs[0x08]);
	printf("%s%+03d.%02d ", notea, octa, centa);
	printf("%10.3f (%04X)\n", fa, pa);
	printf("BTONEPLO= %02X  BTONEPHI= %02X  BAMP= %02X     freq= ",
		s->regs[0x02], s->regs[0x03], s->regs[0x09]);
	printf("%s%+03d.%02d ", noteb, octb, centb);
	printf("%10.3f (%04X)\n", fb, pb);
	printf("CTONEPLO= %02X  CTONEPHI= %02X  CAMP= %02X     freq= ",
		s->regs[0x04], s->regs[0x05], s->regs[0x0a]);
	printf("%s%+03d.%02d ", notec, octc, centc);
	printf("%10.3f (%04X)\n", fc, pc);
	printf("  NOISEP= %02X    ENABLE= %02X               freq= ",
		s->regs[0x06], s->regs[0x07]);
	printf("%s%+03d.%02d ", noten, octn, centn);
	printf("%10.3f (%02X)       mixer= %s\n", fn, pn, mixer);
	printf("  ENVPLO= %02X    ENVPHI= %02X  ENVSC= %02X    freq= ",
		s->regs[0x0b], s->regs[0x0c], s->regs[0x0d]);
	printf("%s%+03d.%02d ", notee, octe, cente);
	printf("%10.3f (%04X)  envelope= %s [%s]\n",
		fe, pe, envelope, envelope_shape[s->regs[0x0d] & 0x0f]);
}
#endif

#ifdef YAMARI

static void print_hex_string(YMF262_State *s, unsigned int offset, unsigned int count)
{
	unsigned int i;
	printf("%03X:", offset);
	for (i = 0; i < count; i++) {
		printf(" %02X", s->regs[offset+i]);
	}
}

static void print_OPL3(YMF262_State *s)
{
	print_hex_string(s, 0x001, 1);
	printf("\t\t\t\t\ttttttttt    t-LSITEST\n");
	print_hex_string(s, 0x002, 1);
	printf("\t\t\t\t\txxxxxxxx    x-TIMER1\n");
	print_hex_string(s, 0x003, 1);
	printf("\t\t\t\t\txxxxxxxx    x-TIMER2\n");
	print_hex_string(s, 0x004, 1);
	printf("\t\t\t\t\trmM___Ss    r-RST m-MT1 M-MT2 s-ST1 S-ST2\n");
	print_hex_string(s, 0x008, 1);
	printf("\t\t\t\t\t_n______    n-NTS\n");
	print_hex_string(s, 0x020, 0x16);
	printf("\tavekmmmm    a-AM v-VIB e-EGT k-KSR m-MULT\n");
	print_hex_string(s, 0x040, 0x16);
	printf("\tkkllllll    k-KSL l-TL\n");
	print_hex_string(s, 0x060, 0x16);
	printf("\taaaadddd    a-AR d-DR\n");
	print_hex_string(s, 0x080, 0x16);
	printf("\tssssrrrr    s-SL r-RR\n");
	print_hex_string(s, 0x0a0, 9);
	printf("\t\txxxxxxxx    x-FNUML\n");
	print_hex_string(s, 0x0b0, 9);
	printf("\t\t__kbbbxx    k-KON b-BLOCK x-FNUMH\n");
	print_hex_string(s, 0x0bd, 1);
	printf("\t\t\t\t\tavrbstch    a-DAM v-DVB r-RYT b-BD s-SD t-TOM c-TC h-HI\n");
	print_hex_string(s, 0x0c0, 9);
	printf("\t\tdcbafffC    d-CHD c-CHC b-CHB a-CHA f-FB C-CNT\n");
	print_hex_string(s, 0x0e0, 0x16);
	printf("\t_____www    w-WS\n");

	print_hex_string(s, 0x101, 1);
	printf("\t\t\t\t\t__tttttt    t-LSITEST\n");
	print_hex_string(s, 0x104, 1);
	printf("\t\t\t\t\t__cccccc    c-CONNECTIONSEL\n");
	print_hex_string(s, 0x105, 1);
	printf("\t\t\t\t\t_______n    n-NEW\n");
	print_hex_string(s, 0x120, 0x16);
	printf("\tavekmmmm    a-AM v-VIB e-EGT k-KSR m-MULT\n");
	print_hex_string(s, 0x140, 0x16);
	printf("\tkkllllll    k-KSL l-TL\n");
	print_hex_string(s, 0x160, 0x16);
	printf("\taaaadddd    a-AR d-DR\n");
	print_hex_string(s, 0x180, 0x16);
	printf("\tssssrrrr    s-SL r-RR\n");
	print_hex_string(s, 0x1a0, 9);
	printf("\t\txxxxxxxx    x-FNUML\n");
	print_hex_string(s, 0x1b0, 9);
	printf("\t\t__kbbbxx    k-KON b-BLOCK x-FNUMH\n");
	print_hex_string(s, 0x1c0, 9);
	printf("\t\tdcbafffC    d-CHD c-CHC b-CHB a-CHA f-FB C-CNT\n");
	print_hex_string(s, 0x1e0, 0x16);
	printf("\t_____www    w-WS\n");
}
#endif

#ifdef EVIE
static void show_Evie(void)
{
	if (EVIE_version != EVIE_NO) {
		...
		print_SID(&sid_state, EVIE_sid_clock_freq);

		AYEMU_read_state(AYEMU_CHIP_EVIE_INDEX, &psg_state);
		print_PSG(&psg_state, EVIE_psg_clock_freq);
	}
}
#endif

#ifdef SIDARI
static void show_SIDari(void)
{
	if (SIDARI_version != SIDARI_NO) {
		RESID_State s;

		printf("Slot: %u  $D5%02X-$D5%02X\n", 
			SIDARI_slot, 
			0x00 + 0x20 * SIDARI_slot, 
			0x1F + 0x20 * SIDARI_slot);

		RESID_read_state(RESID_CHIP_SIDARI_LEFT_INDEX, &s);
		print_SID(&s, SIDARI_clock_freq);
		if (SIDARI_version == SIDARI_STEREO) {
			printf("Second chip:\n");

			printf("Slot: %u  $D5%02X-$D5%02X\n", 
				SIDARI_slot + 1, 
				0x00 + 0x20 * (SIDARI_slot + 1), 
				0x1F + 0x20 * (SIDARI_slot + 1));

			RESID_read_state(RESID_CHIP_SIDARI_RIGHT_INDEX, &s);
			print_SID(&s, SIDARI_clock_freq);
		}
	}
}
...
#ifdef MELODY_PSG
static void show_MelodyPSG(void)
{
	if (MELODY_PSG_enable) {
		AYEMU_State psg_state;

		printf("Base address $D5%02X\n", chip_base_addr);
		printf("Active device #%02X\n", device_index);

		printf("Device #%02X\n", MELODY_PSG_DEVICE);
		printf("Clock %10.3f Hz\n", MELODY_PSG_clock_freq);
		if (MELODY_PSG_model != MELODY_PSG_CHIP_NO) {
			AYEMU_read_state(AYEMU_CHIP_MELODY_PSG_LEFT_INDEX, &psg_state);
			print_PSG(&psg_state, MELODY_PSG_clock_freq);
		}
...
static void show_YAMari(void)
{
	if (YAMARI_enable) {
		...
		printf("Slot: %u  $D5%02X-$D5%02X\n", 
			YAMARI_slot, 
			0x00 + 0x20 * YAMARI_slot, 0x1F + 0x20 * YAMARI_slot);

		YMF262_read_state(YMF262_CHIP_YAMARI_INDEX, &opl3_state);
		print_OPL3(&opl3_state);
	}
}
#endif
...
#ifdef SLIGHTSID
		"SLIGHTSID                      - Display SlightSID registers\n"
#endif
#ifdef EVIE
		"EVIE                           - Display Evie registers\n"
#endif
#ifdef SIDARI
		"SIDARI                         - Display SIDari registers\n"
#endif
#ifdef SONARI
		"SONARI                         - Display SONari registers\n"
#endif
#ifdef MELODY_PSG
		"MELODYPSG                      - Display Melody PSG registers\n"
#endif
#ifdef YAMARI
		"YAMARI                         - Display YAMari registers\n"
#endif
	);
	printf(
...
int MONITOR_Run(void)
{
	...
	for (;;) {
		...
		if (strcmp(t, "CONT") == 0) {
			...
			show_POKEY();
#ifdef SLIGHTSID
		else if (strcmp(t, "SLIGHTSID") == 0)
			show_SlightSID();
#endif
#ifdef EVIE
		else if (strcmp(t, "EVIE") == 0)
			show_Evie();
#endif
#ifdef SIDARI
		else if (strcmp(t, "SIDARI") == 0)
			show_SIDari();
#endif
#ifdef SONARI
		else if (strcmp(t, "SONARI") == 0)
			show_SONari();
#endif
#ifdef MELODY_PSG
		else if (strcmp(t, "MELODYPSG") == 0)
			show_MelodyPSG();
#endif
#ifdef YAMARI
		else if (strcmp(t, "YAMARI") == 0)
			show_YAMari();
#endif
#ifdef MONITOR_ASSEMBLER
